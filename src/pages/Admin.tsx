...
import {
  ...
  Home, Shield, Eye, Trash2, Search, ChevronDown, Bell, Menu, X, ArrowRight
} from "lucide-react";
...
export default function Admin() {
  ...
  const stats = [
    { label: "المستخدمون", value: users.length + 12, icon: Users, color: "from-[#1a5fb4] to-[#3b82f6]", change: "+12%" },
    { label: "السير الذاتية", value: 47, icon: FileText, color: "from-[#065f46] to-[#10b981]", change: "+8%" },
    { label: "الإيرادات", value: "235K", icon: DollarSign, color: "from-[#92400e] to-[#d97706]", change: "+23%" },
    { label: "اليوم", value: 7, icon: TrendingUp, color: "from-[#7c3aed] to-[#a78bfa]", change: "+5%" },
  ];
  ...
  const handleTabChange = (t: Tab) => {
    setTab(t);
    setSidebarOpen(false); // أغلق الـ sidebar على الموبايل عند اختيار تبويب
  };
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc]" dir="rtl">
      ...
      {/* Sidebar */}
      <aside className={`fixed top-0 right-0 h-full w-64 bg-[#0f172a] text-white flex flex-col z-50 transition-transform duration-300
        ${sidebarOpen ? "translate-x-0" : "translate-x-full"} md:translate-x-0 md:static md:h-screen md:flex`}
      ...
      >
        ...
        <div className="flex items-center justify-between p-4 border-b border-white/10 h-16">
          <div className="flex items-center gap-3">
            ...
            <span className="font-bold text-white">لوحة الإدمن</span>
          </div>
        ...
        <main className="flex-1 p-4">
          ...
          {tab === "dashboard" && (
            <div className="space-y-4">
                  ...
                  return (
                    <div key={i} className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className="text-xs text-green-600 bg-green-50 px-1.5 py-0.5 rounded-full font-medium">{s.change}</span>
                      </div>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
                <h2 className="font-bold text-gray-800 mb-3">آخر السير الذاتية</h2>
                <div className="space-y-2">
                  {mockCVs.map(cv => (
                    <div key={cv.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                      <div className="flex items-center gap-2">
                        <div className="w-8 h-8 bg-[#1a5fb4]/10 rounded-lg flex items-center justify-center shrink-0">
                          <FileText className="w-4 h-4 text-[#1a5fb4]" />
                        </div>
                        <div>
                          <p className="font-medium text-sm text-gray-800">{cv.name}</p>
                          <p className="text-xs text-gray-500">{cv.template} · {cv.date}</p>
                        </div>
                      </div>
                      <span className="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded-full shrink-0">{cv.status}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100">
              <div className="p-4 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                <div className="relative">
                  ...
                  <input value={search} onChange={e => setSearch(e.target.value)} placeholder="بحث..." className="border border-gray-200 rounded-lg px-3 py-1.5 pr-9 text-sm w-full sm:w-40 focus:outline-none focus:border-[#1a5fb4]" />
                </div>
              </div>
              <div className="divide-y divide-gray-50">
                {users.filter(u => u.name.includes(search) || u.email.includes(search)).map(u => (
                  <div key={u.id} className="flex items-center justify-between p-3 hover:bg-gray-50">
                    <div className="flex items-center gap-2">
                      <div className="w-8 h-8 bg-gradient-to-br from-[#1a5fb4] to-[#3b82f6] rounded-full flex items-center justify-center text-white text-xs font-bold shrink-0">
                        {u.name.charAt(0)}
                      </div>
                      <div>
                    ...
                    <div className="flex items-center gap-2">
                      <span className={`text-xs px-2 py-0.5 rounded-full hidden sm:block ${u.role === "admin" ? "bg-purple-100 text-purple-700" : "bg-blue-100 text-blue-700"}`}>
                ...
                {users.length === 0 && <div className="text-center py-12 text-gray-400 text-sm">لا يوجد مستخدمون بعد</div>}
              </div>
            </div>
          )}
          ...
          {tab === "cvs" && (
            <div className="bg-white rounded-xl shadow-sm border border-gray-100">
              ...
              <div className="divide-y divide-gray-50">
                {mockCVs.map(cv => (
                  <div key={cv.id} className="flex items-center justify-between p-3 hover:bg-gray-50">
                    <div className="flex items-center gap-2">
                      <div className="w-8 h-8 bg-[#1a5fb4]/10 rounded-lg flex items-center justify-center shrink-0">
                        <FileText className="w-4 h-4 text-[#1a5fb4]" />
                      </div>
            ...
            <div className="space-y-4 max-w-lg">
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
                <h2 className="font-bold text-gray-800 mb-4 flex items-center gap-2">
                  <Settings className="w-5 h-5 text-[#1a5fb4]" />إعدادات الموقع
                </h2>
                <div className="space-y-3">
                  {[
                    { label: "اسم الموقع", value: "CV Pro" },
                    { label: "رقم سيرياتيل كاش", value: "0982493924" },
                    { label: "سعر السيرة الذاتية (ل.س)", value: "5000" },
                    { label: "البريد الإلكتروني للدعم", value: "support@cvpro.sy" },
                  ].map((f, i) => (
                    <div key={i} className="space-y-1">
                      <label className="text-sm font-medium text-gray-700">{f.label}</label>
                      <input defaultValue={f.value} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#1a5fb4]" />
                    </div>
                  ))}
                  <Button className="w-full bg-gradient-to-r from-[#1a5fb4] to-[#3b82f6] text-white font-semibold">حفظ الإعدادات</Button>
                </div>
              </div>

              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
                <h2 className="font-bold text-gray-800 mb-4 flex items-center gap-2">
                  <Shield className="w-5 h-5 text-[#1a5fb4]" />معلومات الحساب
                </h2>
                <div className="space-y-2 text-sm mb-4">
                  ...
                  <div className="flex justify-between py-2 border-b border-gray-100"><span className="text-gray-500">البريد</span><span className="font-medium text-xs">{user.email}</span></div>
                  <div className="flex justify-between py-2"><span className="text-gray-500">الصلاحية</span><span className="bg-purple-100 text-purple-700 text-xs px-2 py-0.5 rounded-full">مدير</span></div>
                </div>
